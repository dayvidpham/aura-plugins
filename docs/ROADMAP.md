...
| 🟡 | **1i. Skill-drift CI check** | (not yet filed) | Add a CI check that flags any drift between `aura-plugins/skills/<skill>/SKILL.md` and `pasture/skills/<skill>/SKILL.md` for the 7 overlapping skills. Migration doc named this; never filed. |
...
**9 prerequisite tasks** — 5 visibility audits (cmvu5 children) + 4 peer
epics implementing R-rows directly.

| Status | Type | Task | Why it blocks `jbnx3` closure |
|---|---|---|---|
| 🚫 | gate | [`aura-plugins-ow0pq`](beads://aura-plugins-ow0pq) | The single direct blocker on `jbnx3`. Re-walks R1–R7 once the 9 below land. |
...
| 🟡 | epic | [`aura-plugins-6ujr`](beads://aura-plugins-6ujr) | **R4** — `aura-acp` plugin for ACP integration. |
...
or administrative): `aura-plugins-kv0od` (multi-vendor; new), `aura-plugins-64mld`
...
| 🟡 | **2h. 30 Python-only skills port to Go** | [`aura-plugins-x5071`](beads://aura-plugins-x5071) | URE §4.5 correction. The "no policy" framing was wrong — `x5071` is the filed policy ("Port all remaining ~30 Python skill directories to Go pasture; user confirmed: Go becomes canonical source"). **Blocks `jbnx3` closure** (R5-adjacent). |
...
| 🟡 | **2j. R4 ACP Integration (Full Client)** | tracked by [`aura-plugins-6ujr`](beads://aura-plugins-6ujr) + [`aura-plugins-rk2su`](beads://aura-plugins-rk2su) | URD `jbnx3` R4. Locally-defined ACP wire types + Adapter interface + static registry + 12 hook events. **Blocks `jbnx3` closure**. |
...
| 🟡 | **2p. UAT-revision code review non-blocking improvements (Python era)** | [`aura-plugins-ad8i1`](beads://aura-plugins-ad8i1) | FOLLOWUP-2 epic from code review of Python-era UAT revision (`oqhjg`). 8 IMPORTANT findings (signal handler, missing unit tests, formatter/enum issues). Findings not yet broken into child tasks. Note: Python `aura_protocol` is deprecated; user should verify which findings still apply under Go (Pasture) before actioning. Not a `jbnx3` closure blocker. Placed §2-active by audit `aura-plugins-3iz51` (2026-05-29). |
| 🟡 | **2q. Beads → Temporal migration (task tracking)** | [`aura-plugins-9wdwc`](beads://aura-plugins-9wdwc) | Deferred from aurad+aura-msg UAT. User verbatim: *"We should also plan for how we will move away from Beads. Beads is highly unstable and quickly turning into a garbage fire."* Scope TBD in future URE; depends on aurad+aura-msg being complete. Not a `jbnx3` closure blocker. Placed §2-active by audit `aura-plugins-3iz51` (2026-05-29). |
